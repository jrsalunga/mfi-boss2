...
$cleanUrl->setParts('isdhdrid');

//echo $isdhdrid;
if(is_uuid($isdhdrid)){
	$isdhdr = vIsdhdr::find_by_id($isdhdrid);
	if(!$isdhdr){
		$isdhdr = vIsdhdr::first('refno');
	}
} else {
	$isdhdr = vIsdhdr::first('refno');
}
...
<head>
<meta charset="utf-8">
<title>Direct Material Issuance : <?=$isdhdr->refno?></title>
...
<div id="page-wrap">
	<div class="isposted" style="visibility: <?=$isdhdr->posted==1?"visible":"hidden"?>">
    	<h1>Posted</h1>
        <p><?=short_date(Stockcard::field_row('txnrefno', $isdhdr->refno, 2))?></p>
    </div>
    <div class="iscancelled" style="visibility: <?=$isdhdr->cancelled==1?"visible":"hidden"?>">
    	<h1>Cancelled</h1>
    </div>
    <div id="header">
    	<div id="main-logo">
    	...
    	<div id="header-wrap">
            ...
            <h1 class="reportLabel">Direct Material Issuance</h1>
        </div>		
    </div>
    <div id="body">
   		<div id="m-container">
   			<div id="hdr">
                ...
                <?php
					#$location = Location::find_by_id($isdhdr->locationid);
				?>
                <div></div>
                </div>           	
                <table id="meta">
                	<tbody>
                    	<tr>
                        	<td>Reference #</td><td><?=$isdhdr->refno?></td>
                        </tr>
                        <tr>
                        	<td>Date</td><td><?=short_date($isdhdr->date)?></td>
                        </tr>
                    
                        <tr>
                        	<td>Project</td><td><?=Project::row($isdhdr->projectid,1)?></td>
                        </tr>
                        <tr>
                        	<td>Operator</td><td><?=Operator::row($isdhdr->operatorid,1)?></td>
                        </tr>
                        <tr>
                            <td>Branch</td><td><?=Branch::row($isdhdr->branchid,1)?></td>
                        </tr>
                    </tbody>
                </table>
            ...
            <table id="items">
                ...
                <tbody>
                	<?php
					$rcpdtls = vIsddtl::find_all_by_field_id('isdhdr',$isdhdr->id);
                    $totamount = 0;
        ...
        <div style="margin: 0 20px 50px;"><strong>Notes:</strong> <em><?=$isdhdr->notes?></em></div>
    </div>
...
<div id="settings-dialog" class="show">
    ...
    <?php
		$n = vIsdhdr::next('refno', $isdhdr->refno);

		$p = vIsdhdr::previous('refno', $isdhdr->refno);
        //global $database;
        //echo $database->last_query;
	?>
    <div class="pager-c">
    	<ul class="pager">
          <li class="previous">
          	<?=$p?'<a href="/reports/print-direct-issuance/'.$p->id.'">Prev</a>':'<span class="disabled">Prev</span>'?>
          </li>
          <li class="next">
          	<?=$n?'<a href="/reports/print-direct-issuance/'.$n->id.'">Next</a>':'<span class="disabled">Next</span>'?>
          </li>
        </ul>
    </div>
